...
**Status:** ✅ Complete & verified (55/55 unit tests passing)
...
with empty content, invisibly polluting downstream chunking/embedding.
...
| Scanned PDF **discovered in a directory listing** | Skipped with a `log:printWarn` (like other non-text content); the walk continues |
...
- **`ballerina/utils.bal`** — `isScannedPdfError` recognises the sentinel phrase
  (`"no extractable text layer"`), the same message-matching pattern as `isEmptyListing`.
- **`ballerina/file_data_loader.bal`** — `listDirectory` catches a scanned-PDF error from
